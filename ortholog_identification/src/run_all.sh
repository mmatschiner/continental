# m_matschiner Thu Sep 13 21:24:33 CEST 2018

# Copy the zebrafish query file.
# bash prepare_query.sh # Not necessary, file is placed in data directory.
...
# Remove alt scaffolds from the zebrafish reference.
bash remove_alt_scfs_from_danrer.sh
...
# Filter genes in directory 07 by their number of exons and the distances between these on the danrer genome assembly.
...
# Check all alignments manually. Gene IDs of unreliable alignments are written to ../res/manual/tables/unreliable_gene_ids.txt

# Selection genes based on stats and the manual check.
