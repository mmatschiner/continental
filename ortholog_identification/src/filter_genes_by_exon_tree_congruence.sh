...
exon_info_table_in=../data/tables/nuclear_queries_exons_miss2_databases14.txt
...
do
	...
	sbatch --account ${acct} -o ${out} filter_genes_by_exon_tree_congruence.slurm ${indir} ${last_char} ${second_last_char} ${outdir} ${exon_info_table_in} ${exon_info_table_out} ${min_n_exons_per_gene} ${raxml_bin} ${concaterpillar_dir} ${n_cpus}
    done
done