# m_matschiner Wed Feb 6 17:42:24 CET 2019
...
unless starbeast
    ...
    xml_string << "#{first_tab}\t<operator id=\"treeSubtreeSlide:Species\" spec=\"SubtreeSlide\" tree=\"@tree.t:Species\" weight=\"25.0\"/>\n"
    xml_string << "#{first_tab}\t<operator id=\"treeExchange:Species\" spec=\"Exchange\" tree=\"@tree.t:Species\" weight=\"25.0\"/>\n"
    xml_string << "#{first_tab}\t<operator id=\"treeNarrowExchange:Species\" isNarrow=\"false\" spec=\"Exchange\" tree=\"@tree.t:Species\" weight=\"25.0\"/>\n"
    xml_string << "#{first_tab}\t<operator id=\"treeWilsonBalding:Species\" spec=\"WilsonBalding\" tree=\"@tree.t:Species\" weight=\"25.0\"/>\n"
    xml_string << "\n"
end
...
if ucln_clock
    ...
    if starbeast
        gene_tree_ids.each do |g|
            xml_string << "#{first_tab}\t<operator id=\"relaxedStdevScaler:#{g}\" parameter=\"@ucldStdev.c:#{g}\" scaleFactor=\"0.8\" spec=\"ScaleOperator\" weight=\"10.0\"/>\n"
        ...
        xml_string << "#{first_tab}\t<operator id=\"relaxedStdevScaler:Species\" parameter=\"@ucldStdev.c:Species\" scaleFactor=\"0.8\" spec=\"ScaleOperator\" weight=\"10.0\"/>\n"
...
if write_slurm
    ...
    slurm_string << "\n"
    slurm_string << "# Job name:\n"
    slurm_string << "#SBATCH --job-name=#{analysis_id[0..7]}\n"
    slurm_string << "#\n"
    slurm_string << "# Project:\n"
    slurm_string << "#SBATCH --account=nn9244k\n"
    slurm_string << "#\n"
    slurm_string << "# Wall clock limit:\n"
    slurm_string << "#\n"
    slurm_string << "#SBATCH --time=168:00:00\n"
    slurm_string << "#\n"
    slurm_string << "# Processor and memory usage:\n"
    slurm_string << "#SBATCH --ntasks-per-node=4\n"
    slurm_string << "#SBATCH --nodes=1\n"
    slurm_string << "#SBATCH --mem-per-cpu=5G\n"
    slurm_string << "#\n"
    slurm_string << "# Outfile:\n"
    slurm_string << "#SBATCH --output=#{analysis_id}.out\n"
    slurm_string << "\n"
    slurm_string << "## Set up job environment:\n"
    slurm_string << "source /cluster/bin/jobsetup\n"
    slurm_string << "module load beast2/2.5.0\n"
    slurm_string << "module load beagle-lib/2.1.2\n"
    slurm_string << "\n"
    slurm_string << "# Run beast.\n"
    slurm_string << "if [ ! -f #{analysis_id}.log ]\n"
    slurm_string << "then\n"
    slurm_string << "  beast -threads 3 -seed #{rand(100000)} -beagle #{analysis_id}.xml\n"
    slurm_string << "else\n"
    slurm_string << "  beast -threads 3 -seed #{rand(100000)} -beagle -resume #{analysis_id}.xml\n"
    slurm_string << "fi\n"
    slurm_start_file_name = "#{output_file_directory}start.slurm"
    slurm_start_file = File.new(slurm_start_file_name,"w")
    slurm_start_file.write(slurm_string)
